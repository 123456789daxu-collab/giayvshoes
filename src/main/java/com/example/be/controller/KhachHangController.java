...
public class KhachHangController {
    ...
    public ResponseEntity<?> uploadAvatar(@PathVariable("id") Long id, @RequestParam("file") MultipartFile file) {
        try {
            ...
            String filename = "kh_avatar_" + id + "_" + System.currentTimeMillis() + "_" + file.getOriginalFilename();
            java.nio.file.Path filePath = uploadPath.resolve(filename);
            java.nio.file.Files.copy(file.getInputStream(), filePath);
            
            // Update entity using service/repository
            khachHangService.updateAvatar(id, "/images/avatars/" + filename);
            
            return ResponseEntity.ok(Map.of("message", "Tải ảnh thành công", "anhDaiDien", "/images/avatars/" + filename));
        } catch (Exception e) {
            return ResponseEntity.status(500).body(Map.of("message", "Lỗi tải ảnh: " + e.getMessage()));
        }
    }
}
